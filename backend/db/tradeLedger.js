...
const brainPaths = require('../brainPaths');

class TradeLedger {
  constructor() {
    this.enabled = process.env.ENABLE_TRADE_LEDGER !== 'false';
    this.dbPath = brainPaths.getLedgerPath();
    this.db = null;
  }

  /**
   * Helper: Run PRAGMA statement (promise-based)
   * @private
   */
  async runPragma(sql) {
  ...
  async initialize() {
    ...
    try {
      ...
      await this.runPragma('PRAGMA synchronous=NORMAL;');
      await this.runPragma('PRAGMA busy_timeout=5000;');

      // Create tables if they don't exist
      await this.createTable();
      await this.ensureLearningColumns();
      await this.createWorkerHeartbeatsTable();
   ...
   * Includes learning columns (instrument, setup_name, mode, timeframe, entry/exit, r_multiple, regime, volatility_score, spread_at_entry, win_loss)
   */
  createTable() {
    return new Promise((resolve, reject) => {
      const sql = `
        ...
        CREATE INDEX IF NOT EXISTS idx_trades_win_loss ON trades(win_loss);
      `;
  ...
  /**
   * Ensure learning columns exist (migration for existing DBs).
   * Idempotent: ignores "duplicate column name".
   */
  async ensureLearningColumns() {
    const columns = [
      ['instrument', 'TEXT'],
      ['setup_name', 'TEXT'],
      ['mode', 'TEXT'],
    ...
    for (const [name, type] of columns) {
      try {
        await new Promise((resolve, reject) => {
          this.db.run(`ALTER TABLE trades ADD COLUMN ${name} ${type}`, (err) => {
            if (err) reject(err);
            else resolve();
          });
        });
        ...
        if (!e.message || !e.message.includes('duplicate column name')) {
          console.warn(`⚠️  Trade ledger ensureLearningColumns ${name}:`, e.message);
        }
      }
    }
    const indexes = [
    ...
    for (const sql of indexes) {
      try {
        ...
        if (!e.message || !e.message.includes('already exists')) {
          console.warn(`⚠️  Trade ledger ensureLearningColumns index:`, e.message);
        }
      }
    }
  }
  ...
  async insertTrade(trade) {
    ...
    const instrument = trade.instrument ?? trade.symbol ?? null;
    ...
    return new Promise((resolve, reject) => {
      const sql = `
        INSERT INTO trades (
          ...
          instrument, setup_name, mode, timeframe, entry_price, exit_price,
          r_multiple, regime, volatility_score, spread_at_entry, win_loss
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `;
  ...
  async updateTradeStatus(tradeId, status, additionalData = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (additionalData.exit_price !== undefined) {
        updates.push('exit_price = ?');
        values.push(additionalData.exit_price);
      }
      if (additionalData.win_loss !== undefined) {
        updates.push('win_loss = ?');
        values.push(additionalData.win_loss);
      }
      if (additionalData.r_multiple !== undefined) {
        updates.push('r_multiple = ?');
        values.push(additionalData.r_multiple);
      }
  ...
  /**
   * Create worker_heartbeats table for cross-service heartbeat tracking
   */
  createWorkerHeartbeatsTable() {
    return new Promise((resolve, reject) => {
      const sql = `
  ...
  /**
   * Upsert worker heartbeat (INSERT ... ON CONFLICT DO UPDATE)
   * @param {string} service - Service name (e.g., 'learningDaemon')
   * @param {object} heartbeat - Heartbeat data
   * @returns {Promise<void>}
   */
  async upsertWorkerHeartbeat(service, heartbeat) {
    if (!this.enabled) {
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      const sql = `
        INSERT INTO worker_heartbeats (service, ts, pid, uptimeSec, payload)
        VALUES (?, ?, ?, ?, ?)
        ON CONFLICT(service) DO UPDATE SET
          ts = excluded.ts,
          pid = excluded.pid,
          uptimeSec = excluded.uptimeSec,
      ...
      const values = [
        ...
        heartbeat.uptimeSec || Math.floor(process.uptime()),
        heartbeat.payload ? JSON.stringify(heartbeat.payload) : null
      ];
  ...
  /**
   * Get latest worker heartbeat by service name
   * @param {string} service - Service name (e.g., 'learningDaemon')
   * @returns {Promise<object|null>} - Heartbeat data or null if not found
   */
  async getWorkerHeartbeat(service) {
    if (!this.enabled || !this.db) {
      return null;
    }
    ...
    return new Promise((resolve, reject) => {
      this.db.get(
        'SELECT * FROM worker_heartbeats WHERE service = ?',
        [service],
        (err, row) => {
          if (err) {
            reject(err);
