...
class OANDABrokerAdapter extends BrokerAdapter {
  ...
  async healthCheck() {
    try {
      ...
      return {
        ...
        accountId: this.accountId ? '***' + String(this.accountId).slice(-4) : 'not set',
        balance: account.balance,
  ...
  supportsPaperMode() {
    return String(this.environment || 'practice').toLowerCase() !== 'live';
  }

  /**
   * TODO: map OANDA pending orders endpoint when needed for reconciliation UI.
   */
