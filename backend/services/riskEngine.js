 ...
 *
 * V1.1 runtime kill switch:
 * - Only TRADING_ENABLED is re-read from process.env on each isTradingEnabled() / validateOrder kill check.
 * - All other env-derived limits on this class remain snapshot at construction (no general hot-reload).
 */
 ...
 * @returns {boolean}
 */
function readTradingEnabledFromEnv(env = process.env) {
...
class RiskEngine {
  constructor() {
    ...
    // Boot-time snapshot (for diagnostics only; decisions use readTradingEnabledFromEnv()).
    this.tradingEnabled = readTradingEnabledFromEnv(process.env);
  ...
  /**
   * Check if trading is enabled (kill switch) — reads TRADING_ENABLED at call time.
   */
  isTradingEnabled() {
    return readTradingEnabledFromEnv(process.env);
  }
